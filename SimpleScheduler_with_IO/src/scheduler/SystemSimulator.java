...
public void run()
  {
    ...
    while (jobsRemainToBeSubmitted || myScheduler.hasJobs()) {  //COME BACK HERE!
    	/* 
		...
		//System.out.println("I think this is finished - TO_DO Finish SystemSimulator.run()");
		 ...
		 */
		//Brianna did this part
		try {
			myScheduler.getRunningJob().getMyCondition().await();
		} catch (InterruptedException e) {
			System.out.println("Interrupted! Boo! Hiss!");
			e.printStackTrace();
		} 
  ...
  public void AddNewProcess(String name, LinkedList<Integer> burstDescriptions, JobWorkable workToDo)
  {
	  Job newJob = new Job(burstDescriptions, this, name, workToDo);
  ...
  public void doIO(int msec) {
	  //send a signal to the Condition, because the kernel should be blocked on a matching await()
  }
