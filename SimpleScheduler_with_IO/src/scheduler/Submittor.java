...
class Submittor extends Thread {
  ...
  public void run() {
    ...
    for (String jobDesc : myJobDescs) {
    	...
    	LinkedList<Integer> burstDescriptions = null;
    	//String burstDescription;  // The description of that Job.  (For FCFS this will be a single integer token)
    	
    	//System.out.println("Think I've finished this part - TO_DO Complete Submittor.run()");
    	/*
    	 * Provide code that will set id, delay, and burstDescription from jobDesc.
    	 * 
    	 */
    	st = new StringTokenizer(jobDesc); //String jobName, number delay, number burst
    	String[] jobElement = new String[3];
    	int i = 0;
    	
    	while (st.hasMoreElements()) {
    		jobElement[i] = (String)st.nextElement();
    		i++;
    	}
    	
    	id = jobElement[0];
    	delay = Integer.parseInt(jobElement[1]);
    	//burstDescription = jobElement[2];
    	StringTokenizer st2 = new StringTokenizer(jobElement[2]); //take contents of last element and split them up to add them to an ArrayList
    	int j = 0;
    	
    	while (st2.hasMoreElements()) {
    		burstDescriptions.add((Integer) st2.nextElement());
    	}
      ...
      mySystem.AddNewProcess(id, burstDescriptions, myWorkCreator.createWork());
    }
